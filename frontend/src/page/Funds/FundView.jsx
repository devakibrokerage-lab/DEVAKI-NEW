...
const Tab = ({ active, label, onClick }) => (
  <button
    onClick={onClick}
    className={`px-4 py-1 rounded-full text-sm font-semibold ${active ? "bg-indigo-600 text-white" : "bg-[var(--bg-card)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]"
      }`}
  >
    {label}
  </button>
...
export default function FundsView() {
  ...
  const fetchFunds = async () => {
    console.log(`[Funds] Fetching for Broker: ${brokerId}, Customer: ${customerId}`);
    setLoading(true);
    try {
      const url = `${apiBase}/api/funds/getFunds?broker_id_str=${brokerId}&customer_id_str=${customerId}`;
      console.log('[Funds] API URL:', url);
      const res = await fetch(url, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const result = await res.json();
      console.log('[Funds] API Response:', result);
      if (result.success && result.data) {
        ...
        console.warn('[Funds] API returned success=false or no data');
      }
    } catch (error) {
      ...
      {userRole === 'customer' && (
        <div className="mt-8">
          <button
            ...
            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition shadow-lg"
          >
