...
import { Search, Trash2, Settings, X } from "lucide-react";
      ...
      >
        <div className="flex justify-between items-center w-full pointer-events-none"> {/* pointer-events-none helps drag work smoothly on text */}
          <div className="min-w-0 flex-1 pr-3">
            <span className="font-medium text-[var(--text-primary)] opacity-90 block truncate">{item.tradingSymbol}</span>
            <span className="text-xs text-[var(--text-secondary)] block mt-0.5 truncate">{item.exchange}{item.name ? ` - ${item.name}` : ''}</span>
          </div>
          <div className="text-right flex-shrink-0">
            <span className="font-semibold text-base sm:text-lg block text-[var(--text-primary)]">{formattedPrice}</span>
            {/* Tinted chip for change indicator with colored text */}
            <span className={`text-[11px] sm:text-xs inline-flex items-center gap-0.5 sm:gap-1 px-1 sm:px-1.5 py-0.5 rounded ${chipBg} ${pnlTextColor}`}>
              {arrow} {formattedNetChange} {formattedPercentChange}
            </span>
            <div className="flex justify-end space-x-2 mt-1">
              <span className="text-[10px] sm:text-xs text-[var(--text-muted)]">Vol: {formattedVolume}</span>
              <span className="text-[10px] sm:text-xs text-[var(--text-muted)] hidden sm:inline">{formattedClose}</span>
            </div>
          </div>
        </div>
      </motion.div>
...
function Watchlist() {
  ...
  useEffect(() => {
    ...
    loadAllInstruments();
  }, [isConnected, apiBase, token, subscribeAndSnapshot, localStorage.getItem('activeContext')]);
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-medium transition disabled:opacity-50"
                >
            ...
            return (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1 py-1.5 text-[11px] font-medium whitespace-nowrap transition-all duration-150 border-b-2 ${isActive
                  ? 'bg-indigo-600/10 text-indigo-400 border-indigo-500 rounded-lg'
